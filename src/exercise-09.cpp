...
int main(void) {
	...
	double **quickSortResults = allocateResults(elementsInArraySize, maxValuesSize);
	...
	for (int i = 0; i < elementsInArraySize; i++) {
		for (int j = 0; j < maxValuesSize; j++) {
			...
			quickSortResults[i][j] = benchmarkSortingAlgorithm(quickSort, size, maxValue, MAX_ITERATION);
		}
	}
	...
	printResults("Quick sort algorithm:", quickSortResults);
	
	freeResults(heapSortResults);
	freeResults(heapSortLinearHeapifyResults);
	freeResults(quickSortResults);

	bool isQuickSortFunctionValid = isSortFunctionValid(quickSort);
	std::cout << "Quick sort function: " << (isQuickSortFunctionValid == true ? "valid" : "invalid") << std::endl;
