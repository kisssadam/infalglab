...
const int sizesToValidateSortFunction[] = {1, 2, 5, 10, 100, 1000, 10000, 100000};
...
int* randomHeap(int size, int maxValue) {
...
double benchmarkHeapSortingAlgorithm(void (*functionToBenchmark)(int, int*), int size, int maxValue, int maxIteration) {
	double elapsedSeconds = 0.0;
	
	for (int k = 0; k < maxIteration; k++) {
		int *heap = randomHeap(size, maxValue);
...
int main(void) {
	...
	for (int i = 0; i < elementsInArraySize; i++) {
		for (int j = 0; j < maxValuesSize; j++) {
			...
			heapSortResults[i][j] = benchmarkHeapSortingAlgorithm(heapSort, size, maxValue, MAX_ITERATION);
	...
	bool isHeapSortFunctionValid = isSortFunctionValid(heapSort);
	bool isMergeSortFunctionValid = isSortFunctionValid(mergeSort);
	
	std::cout << "Heap sort function: " << (isHeapSortFunctionValid == true ? "valid" : "invalid") << std::endl;
